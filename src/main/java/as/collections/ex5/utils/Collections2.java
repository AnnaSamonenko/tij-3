...
public class Collections2 {
    ...
    public static void fillWithFilter(Collection c, Generator gen, int count) {
        for (int i = 0; i < count; i++) {
            Object p = gen.next();
            if (p.toString().substring(0, 1).equals("A"))
                c.add(p);
        }
    }

    public static void fillWithFilter(Map m, MapGenerator gen, int count) {
        for (int i = 0; i < count; i++) {
            Pair p = gen.next();
            if (p.key.toString().substring(0, 1).equals("A"))
                m.put(p.key, p.value);
        }
    }
